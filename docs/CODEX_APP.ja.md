...
この文書は、SPECA を Codex App から実行するための日本語ガイドです。目的は、実際の worker 作業を Codex App / `codex app-server` で実行できるようにすることです。
...
CLI から使う場合も、この fork では runner 未指定で `codex-app` が使われます。
...
別 runtime を使いたい場合だけ、`ORCHESTRATOR_RUNNER` または `--runner` を指定します。
...
`.claude/skills` は Codex prompt へ取り込む元データとして残っていますが、Codex App server の通常経路では `CodexAppRunner` が使われます。
